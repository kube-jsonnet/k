    ...
    horizontalPodAutoscaler:: {
      ...
      mixin:: {
        ...
        spec:: {
          ...
          behavior:: {
            local __behaviorMixin(behavior) = __specMixin({ behavior+: behavior }),
            mixinInstance(behavior):: __behaviorMixin(behavior),
            // scaleDown is scaling policy for scaling Down. If not set, the default value is to allow to scale down to minReplicas pods, with a 300 second stabilization window (i.e., the highest recommendation for the last 300sec is used).
            scaleDown:: {
              local __scaleDownMixin(scaleDown) = __behaviorMixin({ scaleDown+: scaleDown }),
              mixinInstance(scaleDown):: __scaleDownMixin(scaleDown),
              // policies is a list of potential scaling polices which can be used during scaling. At least one policy must be specified, otherwise the HPAScalingRules will be discarded as invalid
              withPolicies(policies):: self + if std.type(policies) == 'array' then __scaleDownMixin({ policies: policies }) else __scaleDownMixin({ policies: [policies] }),
              // policies is a list of potential scaling polices which can be used during scaling. At least one policy must be specified, otherwise the HPAScalingRules will be discarded as invalid
              withPoliciesMixin(policies):: self + if std.type(policies) == 'array' then __scaleDownMixin({ policies+: policies }) else __scaleDownMixin({ policies+: [policies] }),
              policiesType:: hidden.autoscaling.v2beta2.hpaScalingPolicy,
              // selectPolicy is used to specify which policy should be used. If not set, the default value MaxPolicySelect is used.
              withSelectPolicy(selectPolicy):: self + __scaleDownMixin({ selectPolicy: selectPolicy }),
              // StabilizationWindowSeconds is the number of seconds for which past recommendations should be considered while scaling up or scaling down. StabilizationWindowSeconds must be greater than or equal to zero and less than or equal to 3600 (one hour). If not set, use the default values: - For scale up: 0 (i.e. no stabilization is done). - For scale down: 300 (i.e. the stabilization window is 300 seconds long).
              withStabilizationWindowSeconds(stabilizationWindowSeconds):: self + __scaleDownMixin({ stabilizationWindowSeconds: stabilizationWindowSeconds }),
            },
            scaleDownType:: hidden.autoscaling.v2beta2.hpaScalingRules,
            // scaleUp is scaling policy for scaling Up. If not set, the default value is the higher of:
            // * increase no more than 4 pods per 60 seconds
            // * double the number of pods per 60 seconds
            // No stabilization is used.
            scaleUp:: {
              local __scaleUpMixin(scaleUp) = __behaviorMixin({ scaleUp+: scaleUp }),
              mixinInstance(scaleUp):: __scaleUpMixin(scaleUp),
              // policies is a list of potential scaling polices which can be used during scaling. At least one policy must be specified, otherwise the HPAScalingRules will be discarded as invalid
              withPolicies(policies):: self + if std.type(policies) == 'array' then __scaleUpMixin({ policies: policies }) else __scaleUpMixin({ policies: [policies] }),
              // policies is a list of potential scaling polices which can be used during scaling. At least one policy must be specified, otherwise the HPAScalingRules will be discarded as invalid
              withPoliciesMixin(policies):: self + if std.type(policies) == 'array' then __scaleUpMixin({ policies+: policies }) else __scaleUpMixin({ policies+: [policies] }),
              policiesType:: hidden.autoscaling.v2beta2.hpaScalingPolicy,
              // selectPolicy is used to specify which policy should be used. If not set, the default value MaxPolicySelect is used.
              withSelectPolicy(selectPolicy):: self + __scaleUpMixin({ selectPolicy: selectPolicy }),
              // StabilizationWindowSeconds is the number of seconds for which past recommendations should be considered while scaling up or scaling down. StabilizationWindowSeconds must be greater than or equal to zero and less than or equal to 3600 (one hour). If not set, use the default values: - For scale up: 0 (i.e. no stabilization is done). - For scale down: 300 (i.e. the stabilization window is 300 seconds long).
              withStabilizationWindowSeconds(stabilizationWindowSeconds):: self + __scaleUpMixin({ stabilizationWindowSeconds: stabilizationWindowSeconds }),
            },
            scaleUpType:: hidden.autoscaling.v2beta2.hpaScalingRules,
          },
          behaviorType:: hidden.autoscaling.v2beta2.horizontalPodAutoscalerBehavior,
          // maxReplicas is the upper limit for the number of replicas to which the autoscaler can scale up. It cannot be less that minReplicas.
          withMaxReplicas(maxReplicas):: self + __specMixin({ maxReplicas: maxReplicas }),
